import {
  db,
  auth /* BỔ SUNG BIẾN AUTH */,
  onAuthStateChanged /* BỔ SUNG HÀM THEO DÕI TRẠNG THÁI ĐĂNG NHẬP */,
  collection,
  addDoc,
  updateDoc,
  deleteDoc,
...
document.addEventListener("DOMContentLoaded", () => {
  // --- BẢO VỆ TRANG ADMIN CHUYÊN NGHIỆP (LÚC NÀY SẼ CHẠY MƯỢT MÀ) ---
  onAuthStateChanged(auth, (user) => {
    if (!user) {
      // Nếu phát hiện chưa đăng nhập thông qua Firebase, lập tức đẩy về trang chủ
  ...
  const actionVitriContainer = document.getElementById(
    "action-vitri-container",
  );
  ...
  onSnapshot(conferencesCol, (snapshot) => {
    ...
    selectConf.innerHTML =
      '<option value="">-- Vui lòng chọn một Hội nghị --</option>';
    ...
    document
      .querySelectorAll(".delete-conf")
      .forEach((b) =>
        b.addEventListener("click", (e) =>
          deleteConference(e.target.dataset.id),
        ),
      );
  });
  ...
  function renderTabDanhSach() {
    ...
    currentDelegates.forEach((del) => {
      if (currentFilterTab1 !== "ALL" && del.category !== currentFilterTab1)
        return;
  ...
  function renderTabVitri() {
    ...
    currentDelegates.forEach((del) => {
      ...
      if (currentFilterTab3 !== "ALL" && del.category !== currentFilterTab3)
        return;
    ...
    document
      .querySelectorAll(".update-seat-btn")
      .forEach((b) =>
        b.addEventListener("click", (e) =>
          updateDelegateSeat(e.target.dataset.id),
        ),
      );
  }
  ...
  window.updateDelegateSeat = async function (delegateId) {
    const seatValue = document
      .getElementById(`seat-${delegateId}`)
      .value.trim()
      .toUpperCase();
    const conf = currentConferences.find((c) => c.id === selectedConfId);
    if (!conf) return;
    ...
    try {
      await updateDoc(doc(db, "conferences", selectedConfId), {
        seats: updatedSeats,
      });
  ...
  formDel.addEventListener("submit", async (e) => {
    ...
    Swal.fire({
      ...
      position: "top-end",
      icon: "success",
      title: "Đã cập nhật danh sách tổng!",
  ...
  function deleteDelegate(id) {
    Swal.fire({
      title: "Xóa đại biểu?",
      text: "Hành động này sẽ xóa vĩnh viễn đại biểu khỏi danh sách tổng!",
      icon: "warning",
  ...
  formConf.addEventListener("submit", async (e) => {
    ...
    Swal.fire({
      icon: "success",
      title: "Thành công",
      text: "Đã tạo phiên hội nghị mới!",
      timer: 1500,
      showConfirmButton: false,
    });
  });
  ...
  function deleteConference(id) {
    Swal.fire({
      title: "Xóa Hội nghị?",
      text: "Toàn bộ sơ đồ xếp chỗ của hội nghị này sẽ mất!",
      icon: "error",
      showCancelButton: true,
      confirmButtonColor: "#e74c3c",
