...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildAuditoriumMap() {
    ...
    function buildBlock(container, isLeftBlock) {
      rowLetters.forEach((letter, r) => {
        const rowDiv = document.createElement('div'); rowDiv.className = 'auditorium-row';
        const tablesContainer = document.createElement('div'); tablesContainer.className = 'tables-container';
        ...
        if (isLeftBlock) {
          ...
          lbl.textContent = `HÀNG ${letter}`; rowDiv.appendChild(lbl);
        }

        for (let t = 0; t < numTables; t++) {
          const tableGroup = document.createElement('div'); tableGroup.className = 'table-group';
          const chairsRow = document.createElement('div'); chairsRow.className = 'auditorium-chairs';

          for (let c = 0; c < 2; c++) {
            const seat = document.createElement('div'); seat.className = 'seat-3d a-seat';
            const globalSeatIndex = t * 2 + c;
            let seatNum = isLeftBlock ? (seatsInRow - globalSeatIndex) * 2 - 1 : (globalSeatIndex + 1) * 2;
            const seatCode = letter + seatNum;
            seat.dataset.code = seatCode; seat.innerHTML = `<span class="seat-code">${seatCode}</span>`;
            chairsRow.appendChild(seat);
          }
          const tableSurface = document.createElement('div'); tableSurface.className = 'auditorium-table';
          tableGroup.appendChild(tableSurface); tableGroup.appendChild(chairsRow); tablesContainer.appendChild(tableGroup);
        }
        rowDiv.appendChild(tablesContainer); container.appendChild(rowDiv);
      });
    }
    buildBlock(leftBlock, true); buildBlock(rightBlock, false);
  }
  buildAuditoriumMap();
  ...
  onSnapshot(collection(db, 'delegates'), (snapshot) => {
    globalDelegates = {};
    snapshot.forEach(docSnap => { globalDelegates[docSnap.id] = docSnap.data(); });
    if (activeConfId) renderActiveConference();
  });

  onSnapshot(collection(db, 'conferences'), (snapshot) => {
    globalConferences = []; viewConfSelect.innerHTML = '';
    snapshot.forEach(docSnap => { globalConferences.push({ id: docSnap.id, ...docSnap.data() }); });

    if (globalConferences.length === 0) {
      viewConfSelect.innerHTML = '<option>-- Chưa có hội nghị --</option>'; return;
    }

    globalConferences.forEach((conf, idx) => {
      const opt = document.createElement('option'); opt.value = conf.id; opt.textContent = conf.name;
      if (activeConfId === "" && idx === 0) { activeConfId = conf.id; opt.selected = true; }
  ...
  viewConfSelect.addEventListener('change', (e) => { activeConfId = e.target.value; renderActiveConference(); });
  ...
  function renderActiveConference() {
    ...
    document.querySelectorAll('.a-seat').forEach(seat => {
      seat.classList.remove('has-delegate'); seat.removeAttribute('data-tooltip');
    ...
    const seatsToShow = chairmanOrder.filter(code => occupiedVCodes.includes(code));

    if (seatsToShow.length > 0) {
      const cGroup = document.createElement('div'); cGroup.className = 'chairman-group';
      cGroup.style.width = `${seatsToShow.length * 110}px`;
      const cChairs = document.createElement('div'); cChairs.className = 'chairman-chairs';
      const cTable = document.createElement('div'); cTable.className = 'chairman-table';
      ...
      seatsToShow.forEach(seatCode => {
        ...
        const seat = document.createElement('div'); seat.className = 'seat-3d c-seat';
        ...
        slot.appendChild(seat); cChairs.appendChild(slot);
        const panel = document.createElement('div'); panel.className = 'table-panel'; cTable.appendChild(panel);
      });
      cGroup.appendChild(cChairs); cGroup.appendChild(cTable); chairmanContainer.appendChild(cGroup);
    }
    ...
    audienceDelegates.forEach(del => {
      ...
      if (seatEl) {
        ...
        seatEl.setAttribute('data-tooltip', `Đồng chí ${del.name}`);
      }
    });
    ...
    // ==========================================
    const totalChairsCount = document.querySelectorAll('.a-seat').length + seatsToShow.length;
    const emptyCount = totalChairsCount - selectedCount;
    const percent = totalChairsCount > 0 ? ((selectedCount / totalChairsCount) * 100).toFixed(1) : 0;
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ...
      document.querySelectorAll('.seat-3d').forEach(seat => {
        ...
        if (searchTerm !== '' && tooltipText && tooltipText.toLowerCase().includes(searchTerm)) {
          seat.classList.add('highlight-seat');
        } else { seat.boxShadow = ''; seat.classList.remove('highlight-seat'); }
      });
    });
  }

  document.getElementById('btn-refresh-page')?.addEventListener('click', () => { window.location.reload(); });
});